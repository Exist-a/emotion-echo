// Package grpcserver 提供 assessment-svc 的 gRPC 服务（Stage 62 PR-3.2）
//
// 与 HTTP server（Gin :8889）共存：
//   - HTTP 给前端（Nuxt）/ APISIX 网关
//   - gRPC 给内部 BFF（PR-3.3 阶段落地）
//
// 实现 AssessmentService（proto/agent.proto）：
...
// Server assessment-svc 的 gRPC server
type Server struct {
...
func New(svcCtx *svc.ServiceContext, port int) *Server {
	...
	// 注册 AssessmentService
	emotionassessment.RegisterAssessmentServiceServer(gs, &assessmentServer{svcCtx: svcCtx})

	// 注册 health check
	healthSrv := health.NewServer()
	healthSrv.SetServingStatus("", healthpb.HealthCheckResponse_SERVING)
	healthSrv.SetServingStatus("emotion.Assessment", healthpb.HealthCheckResponse_SERVING)
...
func (s *Server) Start(ctx context.Context) error {
	...
	s.listener = lis
	log.Printf("[grpc] assessment-svc gRPC server listening on :%d", s.port)
	log.Printf("[grpc] services: AssessmentService (user id required)")
