...
func TestYaml_HasEnvPlaceholders(t *testing.T) {
	...
	require.Contains(t, body, "${POSTGRES_DSN:-",
		"assessment-api.yaml must use ${POSTGRES_DSN:-...} placeholders")
	require.Contains(t, body, "host=emotion-echo-postgres",
		"assessment-api.yaml must default DSN to container DNS emotion-echo-postgres")

	require.Contains(t, body, "${SKYWALKING_OAP_ADDR:-emotion-echo-sw-oap:11800}",
		"assessment-api.yaml must use ${SKYWALKING_OAP_ADDR:-emotion-echo-sw-oap:11800}")
}
