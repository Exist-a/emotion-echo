...
// 验证 /metrics 端点暴露关键 series + service="assessment-svc" label
package main
...
func TestMetricsEndpoint_AssessmentSvc(t *testing.T) {
	...
	// 断言 4: svc label
	if !strings.Contains(body, `service="assessment-svc"`) {
	...
	healthCounter := readCounterFromBody(t, body, "emotion_echo_http_requests_total", map[string]string{
		"service": "assessment-svc", "method": "GET", "path": "/health", "status": "200",
	})
	if healthCounter < 1 {
