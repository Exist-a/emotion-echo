...
// 验证 /metrics 端点暴露关键 series + service="user-svc" label
package main
...
func TestMetricsEndpoint_UserSvc(t *testing.T) {
	...
	// 断言 4: svc label
	if !strings.Contains(body, `service="user-svc"`) {
	...
	healthCounter := readCounterFromBody(t, body, "emotion_echo_http_requests_total", map[string]string{
		"service": "user-svc", "method": "GET", "path": "/health", "status": "200",
	})
	if healthCounter < 1 {
