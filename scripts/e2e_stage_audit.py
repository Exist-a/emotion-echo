...
# 2026-09-19 波 1 补账后 E2E-03/04/05/06 的 A1/A2/A3/A4/A6 缺口已全部修复，
# 故移除此三阶段的期望（审计器正确报 0 FAIL ≠ 审计器无效）。
# 审计器的检测逻辑仍由下方 A4 合成样本 + row_claims_pass 表驱动 + SELFTEST_MUSTNOT 充分回归。
SELFTEST_EXPECT = {
    # E2E-03/04/05 已于 2026-09-19 波 1 清偿（report 补账 + 状态同步），
    # 原期望 ["A1","A2","A3"] / ["A2","A6"] / ["A4"] 不再适用。
    # 审计器逻辑由合成样本 A4_FIXTURE_ROWS + row_claims_pass_fixture 回归覆盖。
}
