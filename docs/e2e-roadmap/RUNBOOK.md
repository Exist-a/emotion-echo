...
| GitHub 仓库 Settings：为 `main` 建分支保护规则（E2E-03 D1） | **E2E-03 阶段 2** | 用户（需仓库 Settings 权限） | ⚠️ 待确认 |
...
| GitHub token 缺 `workflow` scope | E2E-03 | ✅ **已具备**（2026-09-17 实测：API `x-oauth-scopes` 含 `workflow`；临时分支真实 push `.github/workflows/` 成功，探测分支已清理） |
