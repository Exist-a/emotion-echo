...
| 阻塞项 | 阻塞阶段 | 需谁决定 | 现状 |
|--------|---------|---------|------|
| GitHub token 缺 `workflow` scope / Workflows 写权限（报错 `refusing to allow a Personal Access Token to create or update workflow`） | **E2E-03** | 用户（补 token 权限，或网页手工创建 3 个 workflow 文件） | 🔴 未落定 |
