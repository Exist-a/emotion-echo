...
| **MinIO `localhost:9000` 在 web 容器里不通**（E2E-F-113，✅ 已修 PR #59） | 历史表现：audioUrl 用绝对路径 `http://localhost:9000/...`，宿主的 9000 转发**仅对宿主浏览器生效**；web 容器内 `localhost` 是容器自己 ⇒ `<audio>` 拉不到 metadata | ✅ audioUrl 已改 BFF 反代 `/api/v1/voice/audio/:filekey`（走 APISIX，host 与 PublicBaseURL 解耦，2026-09-22 双视角实测 200）。本行保留作历史教训 |
| **HOST 直跑 `seed.sh` 曾落占位符 secret ⇒ 全路由 401**（E2E-F-114，✅ 已修 PR #61） | 历史表现：`failed to verify jwt`（auth 白名单除外全 401）。根因两层：① 宿主 shell 无 `BFF_JWT_SECRET`；② `services.env.example` plain 赋值 source 时**把显式真值也覆盖成占位符**（故 `source .env.local` 后重跑仍 401） | ✅ seed.sh 已自动 source `deploy/.env.local`（真值优先级 env/services.env > .env.local > example），且 JWT 仍为占位符时打 WARN；example 已改 `${VAR:-}` 模式。若再见 401：`bash deploy/apisix/seed.sh` 重跑（自取真值，无 WARN 即正常） |
| **首次 `/voice/upload` 必失败（5s DeadlineExceeded），重试即成功**（E2E-F-115，🔴 未修） | SenseVoice 空闲后首次推理 > BFF→ai-svc gRPC 5s deadline；BFF 日志 `MultiModalAnalyze latency=5000ms err=DeadlineExceeded`，第二次 `1916ms err=<nil>` | 测试时**第一次录音失败先重试一次再判 FAIL**；根治（调大 deadline / 预热 keepalive）见账本 E2E-F-115 |
