...
**已解除**：`users.status` → 删；注册验证码步骤 → 删；密保可否跳过 → 不可跳过、注册必设；GitHub token `workflow` scope → 已具备（实测真实 push 成功）；**存量未设密保用户 → 选 C 不处理**（数据库现有数据全是无用信息、未上线、测试阶段），改为**新建可重跑、可删除的演示账号**（带密保）。
