...
| 项 | 说明 |
|----|------|
| 测试点 #10 的量化缺口（**已搁置**，见账本 E2E-F-98） | 「AI 回复体体现个性化」原计划做 A/B 对比量化。**已部分完成**：`scripts/verify_personality_prompt_diff.py` 用**对立画像的真实账号**（外向30·神经质6 vs 外向6·神经质30），同一句话、各自新对话，三条消息实测 → 客观特征方向 **5/5 类 × 3/3 条**全中（字数、问句数、安抚词、追问推进词；见 report §4.3）。这些是**确定性文本度量**，不依赖判官，作为本阶段最终验收证据保留。**剩余**："人能感知并被判官识别"层未由 LLM-as-judge 证实 —— 2026-09-21 关账轮用户决议暂搁（详见 §8.2），触发复跑的条件：客观特征度量退化（5/5×3/3 不再全中）或用户反馈"画像没生效"。 |
...
## 8. 后续复核轮追加（2026-09-21）

### 8.1 本阶段验收漏掉一个真实算分 bug（`E2E-F-97`）

复核时按"本阶段还差什么"逐条核对，实测发现 **PHQ-9 / GAD-7 的症状算分是错的**：

| 现象 | 修前实测 | 修后实测 |
|------|---------|---------|
| 全选"完全没有"（正确 score=0，无症状） | **total=9 / mild（轻度）** | **total=0 / none** |
| 选任一题"几乎每天"（正确 score=3） | **HTTP 400** `answer q1 must be 0-3, got 4` | **200，total=27 / extreme** |

根因：前端提交 `option.id`，后端按 `score` 计分。种子数据里 PHQ-9/GAD-7 是 `id 1..4 / score 0..3`
（**id = score + 1**），BIG5 恰好 `id == score` 才一直没暴露。

**为什么本阶段的 14/14 没抓到**：#12「未完成人格量表时 AI 正常回复」与 #4「提交返回五维度分数」
都只测 BIG5（恰好 id==score）；#11「结果持久化」只比对"提交响应 == 列表 == 详情"三者一致，
三者一致并不能证明**数值本身正确**。**同 E2E-F-91 的弱断言模式，但后果更重**
（心理健康筛查对无症状用户报"轻度"）。

**修法与回归钉**：前端改提交 `option.score`（新增 5 条 Playwright **精确取值**用例 + 3 条静态契约）；
同时把验收口径从"形状断言"改成"取值断言"。详见阶段 `docs/e2e-map/` 账本 `E2E-F-97`。

### 8.2 状态恢复 `done`（2026-09-21 关账轮）

**用户决议**：LLM-as-judge 路线暂搁置（"算了，判官工作就暂时不做了，如果后续使用测试中有问题就做一下"）。
原 §8.2 报告的"降 partial"理由（账本留 `E2E-F-98` 判官方法未结）已被本轮解除：

- `E2E-F-98` 状态：🔴 待解决 → ✅ 已关账（决议原文 + 触发复跑的条件已写入账本条目：客观特征度量退化或用户反馈"画像没生效"时重新启用）
- `E2E-F-95` 缺口 ③ 同决议一并搁置 → 🟡 部分解决 → ✅ 已解决
- 客观特征差异（5/5 类 × 3/3 条 + 三条原文对照肉眼可辨）已机械证明，作为本阶段最终验收证据

**14/14 测试点的结论未变**；本轮累计 16/16（含 §8.3 的 2 条新增）。
详档：`docs/legacy-plans/landed/personality-prompt-semantic.md`（frontmatter residuals 已同步更新）。

### 8.3 本阶段新增测试点（补记）

| # | 测试点 | 判定 | 结果 | 证据 |
|---|--------|------|------|------|
| 15 | 症状量表按 `option.score` 计分（两端精确取值） | [A] | PASS | `e2e/survey-scoring.spec.ts` 5/5 + 静态契约 3/3 |
| 16 | 人格量表 `totalScore` 语义显式标注（`scoreKind`） | [A] | PASS | Go 单测 3 条；实测 big5 → `dimension_sum`，PHQ-9/GAD-7 → `risk` |

本节汇总：**PASS 2 / FAIL 0 / BLOCKED 0 / N/A 0**（累计测试点 16 项：§2 的 14 项 + 本节 2 项）
