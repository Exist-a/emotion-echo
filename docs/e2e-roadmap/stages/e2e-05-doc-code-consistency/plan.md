...
| # | 测试点 | 判定 | 验证方式 | 证据 | 结果 |
|---|--------|------|---------|------|------|
| 1 | 10 个脚本本地基线盘点 | [A] | 逐个跑，记录退出码与输出 | 清单 | ⬜ |
| 2 | 全部接入 CI 并在 push 时执行 | [A] | CI 日志出现各脚本步骤 | run URL | ⬜ |
| 3 | 脚本能拦（故意制造 drift） | [A] | 临时改 `apiRoutes.ts` 加一个不存在路由 → CI 红 → revert | 红 run | ⬜ |
| 4 | digest 假绿已修 | [A] | 断言脚本对占位值报错（或显式标注已知缺口） | 输出 | ⬜ |
| 5 | 4 处失真已更正 | [A]+[V] | 逐处 diff 复核 | diff | ⬜ |
| 6 | 更正的失真已登记 ADR-18 表 | [A] | 读 `adr-2026-09-doc-drift-registry.md` | 文档 | ⬜ |
