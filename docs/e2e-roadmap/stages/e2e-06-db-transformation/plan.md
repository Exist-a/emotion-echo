...
| 11 | **演示账号已创建且自带密保** | 跑 seed 脚本 → 登录成功 → 断言该账号密保问题已设置（可答对） | 断言 + DB 查询 | ⬜ |
| 12 | **演示账号 seed 可重跑** | 连续跑两次 seed 脚本，断言不报错、不产生重复账号（幂等） | 两次运行输出 | ⬜ |
| 13 | **演示账号可删除** | 跑清理脚本 → 断言账号从 DB 消失；清理后 dev 环境仍能启动 | 查询输出 | ⬜ |
| 14 | 演示账号删除后既有 spec 依赖已处理 | 若清理后运行 Playwright，spec 应给出明确的前置提示而非莫名失败（或已改为 fixture 动态建号） | 运行输出 | ⬜ |
