...
| # | 测试点 | 判定 | 验证方式 | 证据 | 结果 |
|---|--------|------|---------|------|------|
| 1 | 列表加载且无空白标题 | [A] | 断言每个 item 标题非空（A11 回归钉） | 截图 + 断言 | ⬜ |
| 2 | 时间分组正确 | [A]+[V] | 构造跨天会话，断言分组归属正确（今天/昨天/更早） | 截图 | ⬜ |
| 3 | 点击进入对应会话 | [A] | 断言 URL 与内容匹配（**不是错位的另一个会话**） | 截图 + 断言 | ⬜ |
| 4 | 新建会话 | [A] | 断言创建成功并跳转新会话 | 截图 | ⬜ |
| 5 | 重命名会话 | [A] | 改名后断言列表与详情均更新；刷新后持久 | 截图 + 刷新验证 | ⬜ |
| 6 | 置顶 pin | [A] | pin 后断言排到顶部；刷新后保持；unpin 恢复 | 截图 | ⬜ |
| 7 | 删除会话 | [A] | 断言从列表消失 + 详情不可访问 | 截图 | ⬜ |
| 8 | 删除后的软删除语义（E2E-06 后） | [A] | 断言 DB 行仍在、`deleted_at` 有值；列表/统计不返回 | 断言 | ⬜ |
| 9 | 空态 | [V] | 新账号无会话时的展示（非空白页/非报错） | 截图 | ⬜ |
| 10 | 长标题截断 | [V] | 超长标题断言不破版 | 截图 | ⬜ |
| 11 | 删除/置顶的越权防护 | [A] | 用 A 用户 token 操作 B 用户会话 → 断言拒绝（IDOR，历史 Sprint 112 修过 `userIDQuery` 强依赖） | 断言 | ⬜ |
| 12 | 列表分页/大量数据 | [A]+[V] | 50+ 会话下断言加载正常无卡死 | 截图 | ⬜ |
