...
## 执行路径总览（2026-09-18 收尾后）

```
R-00 审计器 MVA            ✅ 完成（--selftest 通过，含 A4 误报修复与合成样本）
  ▼
R-01 阻断修复              ✅ 完成
  │  · #2 找回密码三层根因全修 + 端到端实测（错答案 401 / 正确答案 200 / 未知用户 401）
  │  · 测试可编译（go vet 7/7）；migrate.sh checksum 负向用例通过；注册按 D-05 恢复
  │  · -race 由"永久降级"改为"查明是真实数据竞争并修复 5 模块"，CI 恢复 -race
  │  · main 写保护自锁死解除；18 个 required checks 接通且红线实测能拦
  ▼
R-02 收口补账              🟡 余 #1~#3（report 模板化 / [V] 截图 / 账本对账）—— 判定为**不补**
  │  已完成：#4 状态三处对齐（E2E-03/04/05/06 → partial）、#5 撤 E2E-06 done、
  │          #6 SSR ADR + decisions.md 决策 24、#7~#14 逐条为真、#15 按 D-06 重做
  ▼
R-03 约束机制收尾          🟡 余 #7（脚本负向测试）/ #10 大项 —— 判定为**不补**
  │  已完成：#5 门禁接通 + 红线实测、#8/#16 两条缺失断言、审计器 A4 修复、
  │          TDD 门禁覆盖范围显式化、密钥扫描器（3 规则 + 负向测试 + CI）
  ▼
E2E-07 找回密码            ✅ 可开工（唯一现场前置：造一个"已设密保"的账号，见 README 交接说明）
  ▼
E2E-08 … E2E-30            按 roadmap 推进；R-02/R-03 余项可并行
```

**出口条件核对（R 系列）**：4 个 workflow 全绿 ✅；密保校验有负向测试（BFF + user-svc 两层）✅；
门禁实测能拦 ✅；§13.3 17 条里 13 条机械化、⑯ 有明确人工理由 ✅；
审计器对 E2E-03/04/05/06 仍报 A1/A2/A3/A4/A6 —— 属**已判定不补**的真实欠账，非未竟的阻塞项。

## 判定记录：哪些不补（2026-09-18，owner 授权"非必须则不补"后由执行方判定）
...
| 项 | 判定 | 理由 |
|----|------|------|
| **E2E-F-69 密钥明文** | ✅ **必要，已补** | 违反 AGENTS.md §四红线；公开仓库 + admin API 曾监听 0.0.0.0 ⇒ 网关完全接管；JWT 密钥泄露 ⇒ 任意用户伪造。且扫描器上线即抓出两处人工漏掉的 .example 明文 |
| **E2E-F-56 / R-03 #10 中的 `-race`** | ✅ **必要，已补** | 不是"覆盖率不达标"这类整洁问题，而是 5 个模块的**真实数据竞争** |
| **R-03 #10 中的 B2（typecheck 进 CI）** | ✅ **必要，已补** | E2E-04 的假 PASS 之一正是"声称 typecheck 已接入而实际没有"；接入后该条由假变真（3 行改动） |
| **R-03 #10 中的 D3（secret 扫描）** | ✅ **必要，已补** | 与 E2E-F-69 配对：只修不防，同类必然复发（本项目已反复验证这一点） |
| **R-02 #1~#3**（report 模板化 / `[V]` 截图 / 账本对账） | ❌ **不补** | 状态已诚实（`partial` + 指向本文件的链接），审计器的 FAIL 是**正确信号**而非危害；补它们要重跑 13 个测试点并拍真实截图，收益是整理已被后续阶段取代的历史 |
| **R-03 #7**（48 个脚本补负向测试） | ❌ **不补（批量）** | 缺口已由 TDD 门禁**显式枚举**（不再静默）。若日后要做，入口应是"破坏性脚本"子集（seed/cleanup 类），而非全量 |
| **R-03 #10 的 A4/A5/B3/B4/C2/C3/D4** | ❌ **不补** | A4 覆盖率 / A5 23 个集成测试 / B3 Playwright 进 CI / B4 构建验证 属大工程且当前无对应危害；C2/C3（models pytest + 覆盖率）依赖私有模型镜像，环境不可得 |
| **R-03 #10 的 A6/B5/C1/D2** | ❌ **不补** | 纯卫生项（格式检查 / engines 字段 / 锁版本 / README 措辞），无当前危害 |
| **charts/ 的 `dev-bff-secret` 默认值** | ❌ **不补** | 决策 23 已冻结 Helm 且从不部署；该值本身是公开的 dev 默认值（非真实凭据），且真实 JWT 密钥已轮换 |
| **R-02 #1~#3**（report 模板化 / `[V]` 截图 / 账本对账） | 🔄 **改判为「补」**（2026-09-19） | 原判"不补"的代价被低估：**正因为没补，E2E-07/08/09/10 四个阶段收口时又犯了同一套错**（0 张截图 / report 非模板 / status 未同步，账本 E2E-F-90）。原判定假设"审计器报 FAIL 是正确信号，无实际危害"，但事实是**信号没被任何人消费**——它们所在阶段的 PR 全部 CI 全绿着合并。故改为补，排期见 [debt-paydown-plan.md](debt-paydown-plan.md) 波 1/波 3。**原行保留**（账本只增不删的同一原则） |

> **判定记录的修订史（2026-09-19）**：上表由 2026-09-18 执行方按 owner 授权"非必须则不补"判定。用户 2026-09-19 指令「将之前的旧账尝试消一下」构成新的 owner 决定，据此 **R-02 #1~#3 改判为「补」**，其余各行的"不补"结论本轮维持不变（理由见 [debt-paydown-plan.md](debt-paydown-plan.md) §4 波 4）。
