...
#### 2026-09-11 增补（本会话我向用户汇报"下一步规划"时产生 2 条类型 5 自报告失真）

用户在收到规划报告后指出"这其中可能有部分已经完成了，你需要查看真实情况"——核对后确认本会话前期规划**多处失真**，均属"未 grep 实证 + 按印象列项"的类型 5 自报告（与 #7 / #10 / #14 / #15 / #18 / #22 / #24 同源）。

| # | 出处 | 文档写的（即我向用户汇报的内容） | 实测事实 | 类型 |
|---|---|---|---|---|
| 36 | 2026-09-11 本会话向用户汇报的"下一步规划档一/档二/档三"清单 | 把 todo-pile-2026-09-04.md §G 节所有项（含已落地项 C6/C5/决策 18 #24/D3 user_oauth/D5 chat-svc 表依赖 ADR/D4 BFF 限流测试）都列为"open / 下步排期"，给出工作量估算与"建议本周/下周做" | (a) **C6 quick-login** 已修（commit `7d8c1d6` PR-4，`pages/login/index.vue:175-196` 改调 `/auth/login` + 注释明写"后端无 /auth/quick-login 端点"）；(b) **C5 QUICKSTART 端口表** 已修（QUICKSTART.md:43 改"BFF (APISIX 后端，dev 调试可直连；prod 仅 APISIX 访问)" + 行 60-63 就地更正块）；(c) **决策 18 #24** 已修（commit `5028e49` + `3e7267e` PR-A：新建 `app/lib/apiBaseUrl.ts` fail-fast helper + 3 处 fallback 字面值 8894/8080 全部移除 + 6 单测覆盖，决策 9 vs 11/12 字面冲突由 `9b03874` 收口）；(d) **D3 user_oauth** 已删（`deploy/db/05-drop-user-oauth.sql` Stage 62 PR-5 + ADR `adr-2026-09-drop-user-oauth-ddl.md`）；(e) **D5 chat-svc 表依赖 ADR** 已出（`adr-2026-09-chat-svc-table-deps.md`）；(f) **D4 BFF 限流测试** 已补（commit `05c5224` Stage 62 PR-1：`recordFailure` 锁定期内 failCount 叠加 bug + 7 helper-only 单测）。todo-pile §G 节就地更正块**已列明所有上述项的落地状态**，本会话前期未读该更正块 | 类型 5 自报告（"按印象列项，未 grep 实证 + 未读 todo-pile §G 节已存在的就地更正块"）。同 #14 / #18 / #22 / #24 型。**教训**：向用户做"下一步规划"类汇报时，**必须先读 todo-pile §G 节就地更正块 + 决策 18 §二 表格**——这两处是已落地项的权威登记位；任何"待做"清单必须显式排除这两处已 close 的项 |
| 37 | 2026-09-11 本会话向用户汇报"最近 5 个阶段（2026-09-11 一日内连续收口）" | 列为 Stage 67/68/69/70/71 五个 | `git log --since="2026-09-11" --oneline` 实测：**当天 6 个 stage 连续收口**（66 B1-B4 收口 / 67 DevEventPublisher / 68 dev dashboard seed / 69 ai-svc x-user-id / 70 Kafka P1 / 71 C8 路由三方对齐）。本会话数成 5 个 | 探测方法错误（同 #6 / #8 型：未实测 git log 数字，凭"印象列出几个最近 stage"）。**教训**：列最近 stage 时必须 `git log --since="<today>"` 实测数量**，不能凭记忆。决策 4.1 的"结论须附可复现命令 + 原始输出"对本会话作者本人同样适用——本条即作者本人失真 |

**修正动作**（本会话后续）：
- 重新核对 todo-pile §G 节、决策 18 §二、git log 实际状态后再做规划汇报（详见修正后的"下一步规划"对话）
- commit `1e8e48d chore(deploy,ai-svc): Stage 70/71 容器化 + config 注释收口` 把未提交改动收口（Stage 71 v0.1.11 镜像号 + Stage 70 config 注释）

附带**累积的失真速率**统计（**Stage 62 后**）：本 ADR **9 天内累积登记 29 条失真 / 5 类成因**（新增类型 6），
其中类型 1（根因臆断）6 条、类型 3（未复跑即记录）9 条、类型 4（探测方法错误）13 条（含 #36 #37 新增）、类型 2（陈旧结论）2 条、类型 5（自报告失真）3 条（#7 #24 #36 #37 共 4 条中 #24 已关闭，本表口径按仍 open 统计 3 条）、**类型 6（破坏性脚本未带默认护栏）2 条**。
...
**类型 5：自报告失真（#7 #10 #14 #15 #18 #22 #24 #36 #37）**——文档的作者/报告者**自己**就是探测者，结论/措辞
...
尤其是那些"如果我错了会导致大返工"的判断。**#36 #37 是本会话作者本人向用户做"下一步
规划"汇报时产生的失真**，补救方式：先读 todo-pile §G 节就地更正块 + 决策 18 §二 表格
（这两处是已落地项权威登记位），再读 git log 实测当前阶段数量。
