...
- 审计日期：2026-08-31
- 关联文档：[architecture-decisions.md](/docs/architecture/decisions.md)（同步修订）、[distributed-roadmap.md](/docs/architecture/roadmap.md)（3.2 条目评审）、[stage-30-apisix-retirement.md](/docs/stages/stage-30-apisix-retirement.md)
...
| [E-2](#e-2-p1--adr-已腐烂) | 🟠 P1 | 工程 | "单一事实源" ADR 停在 7-14，go-zero/APISIX 断言与代码互相矛盾 | ✅ Stage 41 关闭 |
| [T-1](#t-1-p1--前端测试严重偏科) | 🟠 P1 | 工程 | 19 个页面 0 测试、13 组件仅 2 个被测；e2e 仅 1 条 login 用例 | ☐ |
| [E-3](#e-3-p2--依赖卫生) | 🟡 P2 | 工程 | shared go-zero v1.6.0 落后服务 v1.10.x 三个大版本；Python 无锁文件 | ✅ go-zero 部分 Stage 41 关闭（Python 锁文件未触） |
...
**✅ 关闭**：[Stage 41 `docs/stages/stage-41-gozero-removal.md`](../stages/stage-41-gozero-removal.md) 2026-09-07 收尾完成（10 个 TDD PR 全部 merged）。决策 1 的"部分失效"标注行已移除（`decisions.md` 决策 1 改为 ✅ 2026-09-07 Stage 41 落地完成）；6 份 etc/*.yaml + 7 份 go.mod 与决策 1 完全一致；`distributed.md` 迁移表 6 行状态全部更新为 ✅。
...
**✅ 关闭（go-zero 部分）**：[Stage 41 `docs/stages/stage-41-gozero-removal.md`](../stages/stage-41-gozero-removal.md) 2026-09-07 收尾完成。go-zero 三版本（shared v1.6.0 / svc v1.10.2 / BFF v1.10.3）**完全移除**——7 个 Go 模块 `go.mod` 零 zeromicro 命中（PR-0~7 + PR-8 全部 merged）。Python 依赖锁文件部分未触动，留作独立 plan。
