...
| # | 标题 | 来源 | 文件 | 工作量 | 影响 dev |
|---|------|------|------|--------|---------|
| **P0-1** | **BFF 5 个下游 gRPC conn 全部无任何拦截器、无 metadata 透传** | B §7 + D §M3 | `web-bff/main.go:52-54, 266-298` | 1d | 是 |
...
| **P0-3** | **`defer span.EndSpan(nil)` 在 for-loop 内 → consumer span 永远累积、OAP 上每条消息耗时 = 整 consumer goroutine 生命周期（Stage 92/93 核心收益被抵消 50%）** | B §2 + C §14 | `ai-svc/internal/consumer/consumer.go:135-142` + `analytics-svc/internal/kafka/consumer.go:212` | 1d | 是 |
...
| **P0-6** | **`CreateExitSpan` 返回值全 discard → chat-svc producer span 永远不 EndSpan** | B §1 + C §15 | `chat-svc/internal/events/kafka_publisher.go:78-92` | 0.5h | 是（OAP 上 producer span 失效） |
