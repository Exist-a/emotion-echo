...
| `grpc-inter-service-migration.md` | 新增（2026-09-07）→ 2026-09-11 Sprint C/D/E/F1 全落地 | 后端微服务间调用 HTTP→gRPC 改造：3 条已落地（chat→ai / ai→llm / BFF→ai）+ Sprint C/D/E/F1（BFF→user-svc 7/7 方法全 gRPC；BFF→chat/assessment/analytics 全 gRPC）。**决策 4 BFF→4 svc 全方法 100% gRPC 覆盖（user/chat/assessment/analytics 4 svc handler 调用的所有方法走 gRPC）**。剩余 Sprint F2（ai-svc 业务方法 gRPC 化）+ chat-svc PinConversation/StreamMessages（功能未触发）+ #32 chat-svc HTTP 500 bug 根因 |
