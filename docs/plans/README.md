...
| `grpc-inter-service-migration.md` | 新增（2026-09-07）→ 2026-09-11 Sprint C/D 全落地 | 后端微服务间调用 HTTP→gRPC 改造：3 条已落地（chat→ai / ai→llm / BFF→ai）+ Sprint C（shared/pkg/ctxkey 重构，user/assessment/analytics gRPC 端到端通）+ Sprint D（chat-svc gRPC 4 RPC 实现，conversations 走 gRPC 200）。**BFF 默认 Transport=grpc 覆盖全部 4 svc，端到端 4/4 业务全 200**。决策 4 在 BFF→4 svc 链路上**真正生效**。剩余 #32 chat-svc HTTP 端 /api/v1/conversations 500 bug（被 BFF 默认 gRPC 规避，根因待查） |
