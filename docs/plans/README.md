...
| `grpc-inter-service-migration.md` | 新增（2026-09-07）→ 2026-09-11 Sprint C/D/E/F1/F2 全落地 | 后端微服务间调用 HTTP→gRPC 改造：3 条已落地（chat→ai / ai→llm / BFF→ai emotion_query）+ Sprint C/D/E/F1/F2（BFF→user-svc 7/7 方法全 gRPC；BFF→chat/assessment/analytics 全 gRPC；BFF→ai-svc 业务 3 方法 + 路由全 gRPC）。**决策 4 BFF→4 svc 全方法 100% gRPC 覆盖（user/chat/assessment/analytics 4 svc handler 调用的所有方法 + ai-svc 业务方法走 gRPC）**。**2026-09-12 销账**：#32（Stage 64 关闭）、BFF error 映射（Sprint G 关闭）、PinConversation（Stage 72 落地）均已关闭，**唯一残余 = chat-svc StreamMessages**（业务未触发，维持 Unimplemented，见决策 4 ADR §八） |
| `bff-grpc-error-mapping-backlog.md` | 新增（2026-09-11）→ landed Sprint G (commit `f874d3f`) | BFF 全局 gRPC error → HTTP code 映射 backlog：Sprint F2 V3 实测发现 tts/synthesize 走 gRPC Unavailable 时 BFF 误返 502（应 503）。5 个 gRPC client 都用 `fmt.Errorf` 简单 wrap，handler 侧硬编码 `http.StatusBadGateway`，需抽 `mapGRPCError` helper（7 类 gRPC code → HTTP code 映射）+ 5 client 改造 + handler 改造 + docker 端到端回归。半天工作量 |
