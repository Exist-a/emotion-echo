...
| `grpc-inter-service-migration.md` | 新增（2026-09-07）→ 2026-09-11 增补 Sprint D | 后端微服务间调用 HTTP→gRPC 改造：3 条已落地（chat→ai / ai→llm / BFF→ai）+ Sprint C 已落地（shared/pkg/ctxkey 重构，user/assessment/analytics gRPC 端到端通）。剩 Sprint D（chat-svc gRPC 6 RPC 实现 + chat-svc HTTP 端 conversations 500 bug，1-2 天）。完成后 BFF 默认 Transport=grpc 覆盖所有 4 svc，Sprint D 期间用 `deploy/compose.sprint-c-override.yml` 把 CHAT_TRANSPORT=http 绕路 |
