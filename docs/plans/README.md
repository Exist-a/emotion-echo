...
| `grpc-inter-service-migration.md` | 新增（2026-09-07）→ 2026-09-11 增补 Sprint C/D | 后端微服务间调用 HTTP→gRPC 改造：已落地 3 条（chat→ai / ai→llm / BFF→ai），BFF→user/chat/assessment/analytics 仍 HTTP（BFF 默认 `Transport=http` 走 HTTP fallback，gRPC 接线已就绪待 ctxkey 重构 + chat-svc 6 RPC 实现后切回 grpc）。**Sprint C（shared/pkg/ctxkey 重构，解决策 18 #26）+ Sprint D（chat-svc gRPC 6 RPC 实现，解 #27）= 2-3 天**，完成后 BFF 默认 Transport 改回 grpc |
