...
const pageEyebrow = computed(() =>
  route.path.startsWith('/chat/conversation')
    ? 'CONVERSATION'
    : route.path.startsWith('/chat/dashboard')
      ? 'REFLECTION'
      : 'EMOTION ECHO',
)
...
.status-dot {
  ...
  border-radius: 50%;
  box-shadow: 0 0 0 4px var(--ee-primary-soft);
}
...
.page-content {
  ...
  padding: 12px clamp(24px, 4vw, 64px) 40px;
  display: flex;
  flex-direction: column;
  min-height: 0;
  overflow: hidden;
