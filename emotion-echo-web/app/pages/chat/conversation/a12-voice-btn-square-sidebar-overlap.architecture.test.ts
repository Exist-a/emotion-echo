...
const idSrc = readFileSync('./app/pages/chat/conversation/[id].vue', 'utf8')
...
describe('Sprint 111 · A12 voice-btn / sidebar 重叠 / icon-btn 居中修复', () => {
  ...
  it('fold-btn 必须 v-if="!isMenuFolded" (展开时才显示)', () => {
    ...
    const foldBtnBlock =
      indexSrc.match(
        /v-if="[^"]*isMenuFolded[^"]*"[\s\S]*?class="icon-btn fold-btn"[\s\S]*?>/,
      )?.[0] || ''
    expect(
  ...
  it('sidebar-header .icon-btn 必须 border-radius: 50%', () => {
    const sidebarHeaderBlock =
      indexSrc.match(/\.sidebar-header\s+\.icon-btn\s*\{[\s\S]*?\n\}/)?.[0] || ''
    expect(
  ...
  it('fold-btn 必须用 SVG (不能是文字字符 ‹)', () => {
    const foldBtnBlock =
      indexSrc.match(/<button[\s\S]*?class="icon-btn fold-btn"[\s\S]*?<\/button>/)?.[0] || ''
    expect(
      /<svg[\s\S]*?>[\s\S]*?<\/svg>/.test(foldBtnBlock) &&
        !/aria-hidden="true">‹</.test(foldBtnBlock),
  ...
  it('new-btn 必须用 SVG (不能是文字字符 +)', () => {
    const newBtnBlock =
      indexSrc.match(/<button[\s\S]*?class="icon-btn new-btn"[\s\S]*?<\/button>/)?.[0] || ''
    expect(
      /<svg[\s\S]*?>[\s\S]*?<\/svg>/.test(newBtnBlock) &&
        !/aria-hidden="true">\+</.test(newBtnBlock),
  ...
  it('sidebar-expand 必须用 SVG (不能是文字字符 ›)', () => {
    const expandBlock =
      indexSrc.match(/<button[\s\S]*?class="sidebar-expand"[\s\S]*?<\/button>/)?.[0] || ''
    expect(
      /<svg[\s\S]*?>[\s\S]*?<\/svg>/.test(expandBlock) &&
        !/aria-hidden="true">›</.test(expandBlock),
  ...
  it('SVG 图标必须 16x16 viewBox=0 0 24 24 (与 voice-btn SVG 一致)', () => {
    ...
    const svgMatches =
      indexSrc.match(/<svg[^>]*viewBox="0 0 24 24"[^>]*width="16"[^>]*height="16"[^>]*>/g) || []
    expect(
      svgMatches.length >= 3,
